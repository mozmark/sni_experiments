...
public class SNITerminator {
	...
	public SNITerminator(String keystoreString, InetAddress listenAddress,
			int listenPort, InetAddress proxyAddress, int proxyPort) {
	...
	private RefreshingKeyManager reInit(SSLContext sslContext,
			KeyStore ks) throws NoSuchAlgorithmException,
		...
		System.out.println("there are " + keyFactory.getKeyManagers().length
				+ " managers");
		RefreshingKeyManager mgr = new RefreshingKeyManager(
				(X509KeyManager) keyFactory.getKeyManagers()[0]);
	...
	public void start() {
		...
		try {
			...
			KeyStore ks = scs
					.createCertForHost("some.sample.domain.example.com");
				...
				SNIMatcher matcher = new SNIMatcher(
					...
					public boolean matches(SNIServerName serverName) {
						...
						try {
							...
							/*
							 * perhaps, since our key manager is magic, we can
							 * avoid re-initing the whole context and just
							 * replace its mgr attribute with a new one from our
							 * KMF(which is, re-init-ed)
							 */
				...
				Thread requestThread = new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							Socket plainSocket = ProxyConnectSocketFactory
									.GetSocket(host, 443, proxyAddress, proxyPort);

							Socket socket = clientSSLSocketFactory
									.createSocket(plainSocket, null,
